...
const stats = ref({ total_requests: 0, total_actual_cost: 0, total_account_cost: 0, total_paid_balance_cost: 0, total_free_balance_cost: 0, total_free_upstream_cost: 0, total_free_balance_issued: 0, total_free_balance_consumed: 0 })
...
const accountCost = computed(() => Number(stats.value.total_account_cost || 0))
...
// 经营结果必须承担普通用户的全部上游成本（含免费额度）以及管理员自身消耗。
// 管理员成本单独展示，但不应从利润口径中漏掉或重复计算。
const totalUpstreamCost = computed(() => accountCost.value + adminCost.value)
const actualProfit = computed(() => paidRevenue.value - totalUpstreamCost.value)
...
const cards = computed(() => [
  ...
  { key: 'cost', label: t('admin.profit.upstreamCost'), value: money(accountCost.value), color: 'text-orange-600 dark:text-orange-400', hint: t('admin.profit.costHint') },
