...
describe('AppSidebar subscription feature flag', () => {
  it('keeps subscription navigation behind the explicit restoration switch', () => {
    expect(componentSource).toContain('const subscriptionNavigationEnabled = false')
    expect(componentSource).toMatch(/\.\.\.\(subscriptionNavigationEnabled \? \[\s*\{ path: '\/subscriptions'/)
    expect(componentSource).toMatch(/\.\.\.\(subscriptionNavigationEnabled \? \[\s*\{ path: '\/admin\/subscriptions'/)
  })
