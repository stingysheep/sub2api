      ...
      <template v-if="isAdmin">
        ...
        <div class="sidebar-section">
          <template v-for="item in adminNavItems" :key="item.path">
            ...
            <template v-if="item.children?.length">
              ...
              <!-- Children -->
              <div v-if="!sidebarCollapsed && (item.alwaysExpanded || isGroupExpanded(item))" class="mb-1 ml-4 border-l border-gray-200 pl-2 dark:border-dark-600">
                <router-link
        ...
        <div v-if="!authStore.isSimpleMode" class="sidebar-section">
          ...
          <router-link
            ...
            :to="item.path"
            class="sidebar-link mb-1"
            :class="{ 'sidebar-link-active': isActive(item.path), 'sidebar-link-collapsed': sidebarCollapsed, 'sidebar-link-affiliate': item.path === '/affiliate' }"
            :title="sidebarCollapsed ? item.label : undefined"
...
interface NavItem {
   ...
   */
  expandOnly?: boolean
  /** Keep critical admin navigation visible without relying on toggle state. */
...
// Keep subscription data and routes intact; this is the single UI restore point.
const subscriptionNavigationEnabled = false
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/user-ranking', label: t('nav.userRanking'), icon: ChartIcon, hideInSimpleMode: true },
    { path: '/batch-image', label: t('nav.batchImage'), icon: BatchImageIcon, hideInSimpleMode: true, featureFlag: flagBatchImageAccess },
    { path: '/usage', label: t('nav.usage'), icon: ChartIcon, hideInSimpleMode: true },
    { path: '/available-channels', label: t('nav.availableChannels'), icon: ChannelIcon, hideInSimpleMode: true, featureFlag: flagAvailableChannels },
    { path: '/monitor', label: t('nav.channelStatus'), icon: SignalIcon, featureFlag: flagChannelMonitor },
    ...(subscriptionNavigationEnabled ? [
  ...
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/groups', label: t('nav.groups'), icon: FolderIcon, hideInSimpleMode: true },
    {
      ...
      children: [
        ...
        // Admins must retain access to monitor configuration even when the
        // public channel-status switch is disabled for regular users.
    ...
    { path: '/admin/profit', label: t('admin.profit.title'), icon: ChartIcon },
