  ...
  <section class="card p-4" aria-labelledby="group-concurrency-title" :aria-busy="refreshing">
    <div class="flex flex-wrap items-start justify-between gap-3">
      ...
      <div class="flex items-center gap-3">
        ...
        <button type="button" class="btn btn-secondary btn-sm" :disabled="refreshing" @click="refresh">{{ t('common.refresh') }}</button>
      </div>
    </div>
    ...
    <template v-if="snapshot">
      <div class="mt-4 flex flex-wrap items-baseline gap-x-5 gap-y-1 border-b border-gray-100 pb-3 dark:border-dark-700">
        ...
        <span class="text-sm text-gray-600 dark:text-gray-300">{{ t('admin.dashboard.groupConcurrency.total', { count: total }) }}</span>
      ...
      <ul v-if="visibleGroups.length" class="mt-1 max-h-80 divide-y divide-gray-100 overflow-y-auto dark:divide-dark-700" :class="{ 'opacity-60': error }" data-testid="group-concurrency-list">
        <li v-for="group in visibleGroups" :key="group.group_id" class="flex items-center gap-3 py-3">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ group.current_in_use > 0 ? t('admin.dashboard.groupConcurrency.active') : t('admin.dashboard.groupConcurrency.idle') }}<span v-if="group.platform"> · {{ group.platform }}</span></p>
          </div>
...
const total = computed(() => sortedGroups.value.reduce((sum, group) => sum + group.current_in_use, 0))
