...
describe('AccountActionMenu — spark shadow 按钮可见性', () => {
  ...
  it.each(['apikey', 'upstream'] as const)('%s 账号显示上游余额入口', (type) => {
    const account = makeAccount({ type, parent_account_id: null })
    const wrapper = mount(AccountActionMenu, {
      props: { show: true, account, anchorRect },
      attachTo: document.body,
    })
    expect(getBodyText()).toContain('admin.accounts.upstreamBalance.action')
    wrapper.unmount()
  })

  it('OAuth 账号隐藏上游余额入口', () => {
    const wrapper = mount(AccountActionMenu, {
      props: { show: true, account: makeAccount({ type: 'oauth' }), anchorRect },
      attachTo: document.body,
    })
    expect(getBodyText()).not.toContain('admin.accounts.upstreamBalance.action')
    wrapper.unmount()
  })
