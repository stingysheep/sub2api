...
type GenerateRedeemCodesRequest struct {
	...
	BalanceSource service.BalanceSource `json:"balance_source" binding:"omitempty,oneof=free paid"`
...
func (h *RedeemHandler) CreateAndRedeem(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.redeem_codes.create_and_redeem", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		...
		createErr := h.redeemService.CreateCode(ctx, &service.RedeemCode{
			...
			Type:          req.Type,
			Value:         req.Value,
			BalanceSource: service.NormalizeBalanceSource(req.BalanceSource),
...
func (h *RedeemHandler) GetStats(c *gin.Context) {
	...
	if err != nil {
		response.InternalError(c, "Failed to load redeem code statistics")
		return
	}
