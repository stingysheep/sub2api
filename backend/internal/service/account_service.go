...
// TestCredentials is retained for interface compatibility. Credential probes
// must go through AccountTestService, which selects the platform protocol.
func (s *AccountService) TestCredentials(ctx context.Context, id int64) error {
	return fmt.Errorf("test credentials is unsupported; use account test service")
}
