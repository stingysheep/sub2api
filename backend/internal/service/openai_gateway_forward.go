...
func shouldForwardOpenAIResponsesViaRawChatCompletions(account *Account) bool {
	...
	if account.IsCNProvider() {
		// Keep the local Kimi/GLM adaptive Chat Completions path. MiniMax and
		// DeepSeek use their native Responses endpoints; explicit protocol wins.
		switch account.GetAPIProtocol() {
		case APIProtocolChatCompletions:
			return true
		case APIProtocolAdaptive:
			return account.Platform != PlatformDeepseek && account.Platform != PlatformMiniMax
		default:
			return false
		}
	}
