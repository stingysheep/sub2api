...
type AffiliateLeaderboardEntry struct {
	...
	HistoryRebate float64 `json:"history_rebate"`
}
...
type AffiliateDetail struct {
	...
	DisplayRebateRatePercent   float64                     `json:"display_rebate_rate_percent"`
...
type AffiliateService struct {
	...
	consumptionRebateMode bool
}

func (s *AffiliateService) SetConsumptionRebateMode(enabled bool) {
	if s != nil {
		s.consumptionRebateMode = enabled
	}
}
...
func (s *AffiliateService) GetAffiliateDetail(ctx context.Context, userID int64) (*AffiliateDetail, error) {
	...
	if repo, ok := s.repo.(interface {
		ListLeaderboard(context.Context, int) ([]AffiliateLeaderboardEntry, error)
	}); ok {
		leaderboard, _ = repo.ListLeaderboard(ctx, 15)
	}
	displayRate := s.resolveRebateRatePercent(ctx, summary)
	if s.settingService != nil {
		displayRate = s.settingService.GetAffiliateDisplayRebateRatePercent(ctx)
	}
	return &AffiliateDetail{
		...
		DisplayRebateRatePercent:   displayRate,
		Invitees:                   invitees,
...
func (s *AffiliateService) AccrueInviteRebate(ctx context.Context, inviteeUserID int64, baseRechargeAmount float64) (float64, error) {
	if s != nil && s.consumptionRebateMode {
		return 0, nil
	}
	return s.AccrueInviteRebateForOrder(ctx, inviteeUserID, baseRechargeAmount, nil)
}

// AccrueInviteUsageRebate awards a share of positive profit generated by a
// paid-balance request, while reusing the existing eligibility and quota rules.
func (s *AffiliateService) AccrueInviteUsageRebate(ctx context.Context, inviteeUserID int64, paidCost, upstreamCost float64, requestID string) (float64, error) {
	if s == nil || s.repo == nil || !s.consumptionRebateMode || inviteeUserID <= 0 || paidCost <= 0 || upstreamCost < 0 || strings.TrimSpace(requestID) == "" {
		return 0, nil
	}
	profit := paidCost - upstreamCost
	if profit <= 0 || !s.IsEnabled(ctx) {
		return 0, nil
	}
	inviteeSummary, err := s.repo.EnsureUserAffiliate(ctx, inviteeUserID)
	if err != nil || inviteeSummary.InviterID == nil || *inviteeSummary.InviterID <= 0 {
		return 0, err
	}
	...
	if s.settingService != nil {
		if days := s.settingService.GetAffiliateRebateDurationDays(ctx); days > 0 && time.Now().After(inviteeSummary.CreatedAt.AddDate(0, 0, days)) {
			return 0, nil
		}
	}
	rate := s.resolveRebateRatePercent(ctx, inviterSummary)
	rebate := roundTo(profit*(rate/100), 8)
	if rebate <= 0 {
		return 0, nil
	}
	if s.settingService != nil {
	...
	usageRepo, ok := s.repo.(interface {
		AccrueUsageQuota(context.Context, int64, int64, float64, int, string) (bool, error)
	})
	if !ok {
		return 0, nil
	}
	applied, err := usageRepo.AccrueUsageQuota(ctx, *inviteeSummary.InviterID, inviteeUserID, rebate, freezeHours, requestID)
	if err != nil || !applied {
		return 0, err
	}
	return rebate, nil
