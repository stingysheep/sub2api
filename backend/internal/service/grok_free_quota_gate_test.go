...
func awaitGrokFreeQuotaCacheRefresh(t *testing.T, cache *sync.Map, id int64) {
	t.Helper()
	require.Eventually(t, func() bool {
		if _, cached := cache.Load(id); !cached {
			return false
		}
		if root, ok := freeQuotaRefreshInFlight.Load(cache); ok {
			if inFlight, ok := root.(*sync.Map); ok {
				_, active := inFlight.Load(id)
				return !active
			}
		}
		return true
...
func TestFilterGrokFreeQuotaAccountsOnlyBlocksExplicitFreeOAuth(t *testing.T) {
	...
	// Each scheduler owns its cache; leave the gateway's shared map untouched.
	...
	awaitGrokFreeQuotaCacheRefresh(t, &scheduler.grokFreeQuotaGateCache, 1)
...
func TestFilterGrokFreeQuotaAccountsRecoversAfterRollingUsageFalls(t *testing.T) {
	...
	// Wait for the real refresh to finish before forcing expiration; do not
	// remove its coalescing marker while the worker is still running.
	awaitGrokFreeQuotaCacheRefresh(t, &scheduler.grokFreeQuotaGateCache, 1)
