...
func TestAdminAffiliateAnalyticsQueryKeepsGrowthAndRankingScopes(t *testing.T) {
	...
	require.Contains(t, content, "$1::timestamptz AT TIME ZONE $3::text")
	require.Contains(t, content, "$2::timestamptz - interval '1 microsecond'")
	require.Contains(t, content, "u.created_at AT TIME ZONE $3::text")
	require.NotContains(t, content, "AT TIME ZONE $3)")
